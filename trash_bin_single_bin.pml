...
// The vertical ram is not used when the interior of the trash bin is empty.
// G ((ram = compress) -> (trash_uncompressed > 0 || trash_compressed > 0))
ltl ram2 { [](ram == compress -> (trash_uncompressed > 0 || trash_compressed > 0)) }

// The outer door can only be opened if no trash is in it.
...
// The user always eventually has no trash.
...
// Every time the truck is requested for a trash bin, the truck has eventually emptied the bin
